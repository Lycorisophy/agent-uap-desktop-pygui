"""
HistoryStore —— **预测轨迹记忆**（与对话记忆、向量记忆并列的一类「知识」）
================================================================

说明：
- 本模块聚焦 **预测任务** 的索引与 JSON 落盘，不等同于完整「智能体记忆体系」
  （后者还含对话、事实卡片、向量等，见 ``project_store`` / ``vector``）。
- 与 **Harness**：供可视化、回放、对比「预测 vs 实际」使用；写入路径由构造参数决定。

扩展 **记忆分层** 时：可把「事件摘要」或「异常检测结果」作为附加字段挂到
``PredictionRecord``，并在 API 层暴露查询。
================================================================
"""
